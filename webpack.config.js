...
**/
module.exports = {
  entry: path.resolve(__dirname, 'redux-demo4/src/js/index.js'),

  output: {
    path: path.resolve(__dirname, 'redux-demo4/deploy'),
    filename: 'bundle.js'//js文件名加hash
  },
  ...
  devServer: {
    contentBase: './redux-demo4',//本地服务器加载目录
    colors: true,//终端输出结果为彩色
    historyApiFallback: true,//不跳转
    inline: true,//实时刷新
    hot: true
  ...
  plugins: [
    new webpack.BannerPlugin("Copyright abc inc."),//文件顶部加注释
    new HtmlWebpackPlugin({
      template: __dirname + '/redux-demo4/src/html/index.tmpl.html'
    }),
    new webpack.HotModuleReplacementPlugin()//热加载插件
  ]
};